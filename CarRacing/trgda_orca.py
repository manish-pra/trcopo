...
sys.path.insert(0, '..')
import torch
from trcopo_optim import TRPO_ORCA as TRPO
import numpy as np
...
optim_p1 = TRPO(p1, lam = 1, bound=1e-4, esp=0.00005)
optim_p2 = TRPO(p2, lam = 1, bound=1e-4, esp=0.00005)
...
for t_eps in range(num_episode):
    ...
    while np.all(done.numpy()) == False:
        ...
        prev_coll_c1 = coll_c1[remaining_xo]
        ...
        if np.all(done.numpy()) == True or batch_mat_state1.size(0)>3000 or itr>700:# or itr>900: #brak only if all elements in the array are true
            prev_size = batch_mat_state1.size(0)
    ...
    optim_p1.zero_grad()
    lam, improvement, a = optim_p1.step(advantage_mat1, state_gpu_p1, batch_mat_action1_gpu)
    ...
    optim_p2.zero_grad()
    lam, improvement, a = optim_p2.step(-advantage_mat1, state_gpu_p2, batch_mat_action2_gpu)
